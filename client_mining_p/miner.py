...
#     while True:
#         # TODO: Get the last block from the server and look for a new one
#         the_last_block = requests.get(url=node+'/last_block').json()
#         print("last black in chain", the_last_block)
#         new_proof = proof_of_work(the_last_block['last_block'])
...
#         # TODO: We're going to have to research how to do a POST in Python
#         # HINT: Research `requests` and remember we're sending our data as JSON
#         response_obj = requests.post(url=node+'/mine', json=post_data)
